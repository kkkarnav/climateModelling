...
def plot_spell_lengths(lats, lons, percentiles, data_type = 'tmax'):
    data = pd.read_csv('./per_grid_data/tmax_data.csv')
    ...
    results_df.to_csv("./spell_length_data.csv", index = False)
    ...
    for x in percentiles:
        ...
        plt.tight_layout()
        plt.savefig(f"./spell_lengths/num_days_over_{x}_trend.png")
        plt.clf()
        ...
        plt.tight_layout()
        plt.savefig(f"./spell_lengths/spell_length_above_{x}_trend.png")
        plt.clf()
